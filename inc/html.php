<?php
	...
	function displayDays($year, $month, $tabs = 3){
		...
		$path   = s(rtrim($config['path'], "/"));
		$q = $db->query("SELECT DAY(FROM_UNIXTIME(`time`" . DB_OFFSET . ")) as d, " . $_month . " AS m, " . $_year . " AS y, `type`, COUNT(*) AS c FROM `".DTP."tweets` 
			WHERE " . $_year . " = '" . $db->s($year) . "' 
			".$qwhr['and_userid']." 
			AND " . $_month . " = '" . $db->s($month) . "' 
			GROUP BY y, m, d, `type` ORDER BY y ASC, m ASC, d ASC, `type` ASC");
		while($r = $db->fetch($q)){
	...
	function tweetHTML($tweet, $tabs = 4){
		...
		$d  =   $t . "<div id=\"tweet-" . s($tweet['tweetid']) . "\" class=\"tweet" . (($tweet['type'] == 1) ? " reply" : "") . (($tweet['type'] == 2) ? " retweet" : "") . (($isStatus) ? " tweetstatus" : "") . "\">\n" .
				...
				$t . "\t<p class=\"meta\">\n" . $t . "\t\t<a href=\"" . $tweet_status_hlink . "\" class=\"permalink\">" . date("g:i A, M jS, Y", ($rt ? $retweet['time'] : $tweet['time'])) . "</a>\n" .
	...
	function tweetsHTML($q, $mode = "", $tabs = 4){
		...
		if($count > 0){
			...
			if($mode == "day"){
				...
				$pTQ      = $db->query($half . " < '" . $db->s($first) . "' ".$qwhr['and_userid']." ORDER BY `time` DESC LIMIT 1");
				$nTQ      = $db->query($half . " > '" . $db->s($last)  . "' ".$qwhr['and_userid']." ORDER BY `time` ASC LIMIT 1");
				if($db->numRows($pTQ) > 0){
